...
const createContent = (item, user) => {
  const { type, msgId, imgPath, content, isSend, talker } = item
  const username = user.conRemark ? user.conRemark : user.nickname

  if (type === 3 || type === 1048625) {
    // 图片 或 gif 图片
    return <MessageImage msgId={msgId} imgPath={imgPath} />
  }
  if (type === 34) {
    // 语音
  ...
  if (type === 50) {
    ...
    return (
      ...
      >
        ...
        >
          {content.split('_')[2] === 'voice' ? '语音通话' : '视频通话'}
        </div>
      </div>
    )
  }
  ...
  if (type === 16777265) {
    // 包含链接
    const href = content.msg.appmsg.des.data
    return (
      <div
        style={{
          wordBreak: 'break-all',
  ...
  if (type === 419430449) {
    // 转账
    return (
      <MessageTransfer content={content} isSend={isSend} username={username} />
    )
  }
  ...
  if (type === 570425393) {
    ...
    const {
      ...
      link_list: { link },
    } = content.sysmsg.sysmsgtemplate.content_template

    const tempStr = link.reduce(
      (prev, { _attributes: { name }, memberlist, plain }) => {
        let data = memberlist ? memberlist.member.nickname.data : plain.data
        return prev.replace(`$${name}$`, data)
      },
      template,
    )
    return (
  ...
  if (type === -1879048183) {
    ...
    const {
      ...
      title: { data: title },
    } = content.msg.appmsg.hardwareinfo.messagenodeinfo
    return <MessageRunLike title={title} displayusername={displayusername} />
  }
  if (type === -1879048185) {
    // 微信运动
